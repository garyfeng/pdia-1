...
def dropBlocksNotMatching(df, blockPattern,
                 blockKey="BlockCode",
                 verbose=True):
    """ Drop blocks do not match a regular expression

    :param df: eNAEP observable data frame
    :param blockPattern: regex of the block names to keep
    ...
    df = df[idx]
    if verbose:
        print("\nDrop blocks not matching " + blockPattern)
